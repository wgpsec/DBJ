...
class EIScan(object):
    ...
    def get_proxy(self, is_add=False):
        ...
        test_p = requests.get('http://118.24.52.95/get_all/', timeout=3)
        ...
        for p_ip in pr_ip:
            ...
            try:
                ...
                requests.get("http://118.24.52.95/delete/?proxy={}".format(p_ip['proxy']))
    ...
    def get_req(self, url, referer, redirect, is_json=False, t=0):
        ...
        try:
            ...
            requests.get("http://118.24.52.95/delete/?proxy={}".format(proxy['https']))
            if len(self.user_proxy) > 3:
                logger.info("【自动删除代理】 {} ".format(proxy))
                self.user_proxy.remove(proxy)
            if t > len(self.user_proxy) / 2 or len(self.user_proxy) < 3:
    ...
    def export(self, res, eName):
        with open('{}-{}.csv'.format(datetime.date.today(), eName), 'a', newline='', encoding='utf-8-sig') as csvfile:
